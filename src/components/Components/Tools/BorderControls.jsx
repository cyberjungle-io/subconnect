...
const BorderControls = ({ style, onStyleChange }) => {
  ...
  useEffect(() => {
    ...
    if (style.borderRadius) {
      const [topLeft, topRight, bottomRight, bottomLeft] =
        style.borderRadius.split(" ");
      setBorderRadius({
        topLeft,
        topRight: topRight || topLeft,
        bottomRight: bottomRight || topLeft,
        bottomLeft: bottomLeft || topRight || topLeft,
      });
  ...
  const handleAllBorderWidthChange = useCallback(
    (value) => {
  ...
  const handleSingleBorderWidthChange = useCallback(
    (side, value) => {
      setBorderWidth((prev) => {
  ...
  const handleAllBorderRadiusChange = useCallback(
    (value) => {
  ...
  const handleSingleBorderRadiusChange = useCallback(
    (corner, value) => {
  ...
  const handleIndividualInputKeyDown = useCallback(
    (e, side, currentValue, property) => {
      if (e.key === "ArrowUp" || e.key === "ArrowDown") {
        e.preventDefault();
        const step = e.shiftKey ? 10 : 1;
      ...
      <div className="flex">
        <input
          ...
          onChange={(e) => {
            ...
            const newValue =
              (numericValue || "1") + (value?.match(/[a-z%]+/i) || "px");
            setter(newValue);
            if (property === "width") {
              handleSingleBorderWidthChange(side.toLowerCase(), newValue);
            } else {
              handleSingleBorderRadiusChange(
                side.toLowerCase().replace(" ", ""),
                newValue
              );
            }
          }}
          onKeyDown={(e) =>
            handleIndividualInputKeyDown(
              e,
              side.toLowerCase().replace(" ", ""),
              value,
              property
    ...
    <div className="mb-4">
      <div className="flex justify-between items-center mb-2">
        ...
        <button
          onClick={() =>
            label === "Border Width"
              ? toggleAllBorderWidth()
              : toggleAllBorderRadius()
          }
          ...
          <input
            ...
            onKeyDown={(e) =>
              handleKeyDown(
                e,
                label === "Border Width"
                  ? handleAllBorderWidthChange
                  : handleAllBorderRadiusChange,
          ...
          {label === "Border Width" ? (
            <>
              <div className="col-start-2">
                {renderInput("Top", values.top, setters.top, property)}
              </div>
              <div className="col-start-1 row-start-2">
                {renderInput("Left", values.left, setters.left, property)}
              </div>
              <div className="col-start-3 row-start-2">
                {renderInput("Right", values.right, setters.right, property)}
              </div>
              <div className="col-start-2 row-start-3">
                {renderInput("Bottom", values.bottom, setters.bottom, property)}
              </div>
            </>
          ) : (
            <>
